...
/**
 * This class is run when launching the game as a command line application. It
 * provides the link between the player interacting with the computer and the
...
public class CommandLine {
    ...
    /**
     * Repeatedly prints `message` until the input is an integer between minRange
     * and maxRange
     * 
        ...
        do {
            try {
                ...
                if (userNum < minRange || userNum > maxRange)
                    System.out.println("Invalid Range - Must be between %d and %d"
                            .formatted(minRange, maxRange));
                else
                    validInput = true;
    ...
    /**
     * Lists &amp; describes the available ships to the player, returning their
     * choice.<br>
    ...
    public static Ship getValidShipChoice(Ship[] possibleShips) {
        ...
        Ship chosenShip;
        System.out.println(
                "To be a captain, you need a ship! " + "The local shipyard has several available:");
        do {
    ...
    /**
     * Main State Function - Gets the player's name, ship, and game length, then
     * starts the game
     */
    ...
    private static void s_viewCharacteristics() {
        ...
        System.out.println("Crew: %d (%d gold per day)".formatted(num_crew,
                (int) (Ship.DAILY_WAGE * num_crew)));
    ...
    private static void s_viewLedger() {
        ...
        for (Item item : environment.getAllSold()) {
            String salePrice = (item.getSalePrice() == 0) ? "-" : "" + item.getSalePrice();

            System.out.println(item.getName() + "\t" + item.getPurchasedFrom() + "\t"
                    + item.getPurchasedPrice() + "\t" + salePrice);
        }
    }
    ...
    private static void s_visitStore() {
        ...
        System.out.println("You have: %d gold, and %d spare cargo capacity."
                .formatted(environment.getMoney(), environment.getShip().getSpareCapacity()));
        while (stayInStore) {
    ...
    private static void s_wantToSell(Store store) {
        ...
        while (stillHere) {
            ...
            for (Item item : ship.getCargo()) {
                ...
                System.out.println("[%d] %s | %d (%s%d) gold".formatted(++i, name, price,
            ...
            System.out.println("Sold %s for %d gold (%d %s)".formatted(item.getName(), price,
                    Math.abs(priceDiff), (priceDiff < 0 ? "loss" : "profit")));
    ...
    private static void s_embark() {
        ...
        if (money < 0) {
            System.out.println("!!! HALT - You're not allowed to leave while %d gold in debt"
                    .formatted(-money));
            System.out.println("!!! Sell your cargo to get yourself back in the black");
            return;
        }
        ...
        ArrayList<Route> validRoutes = Route.availableRoutes(environment.getAllRoutes(),
                currentIsland);
        int i = 0;
    ...
    private static void ev_pirate(PirateAttack pirate) {
        ...
        case DAMAGED:
            ...
            System.out.println(
                    "The pirates have been repelled, however " + damage + " damage has been taken");
        ...
        case LOSS:
            ...
            for (Item item : lostCargo) {
                ...
                System.out.println(
                        "- %s | %d gold".formatted(item.getName(), item.getPurchasedPrice()));
            }
            ...
            if (satisfied) {
                ...
                System.out.println(
                        "The pirates aren't satisfied with the measly contents of your cargo hold!");
                System.out.println(
    ...
    private static void s_sailing() {
        ...
        if (event instanceof Weather) {
            ...
            System.out.println(
                    "Error: Can't handle RandomEvent of type " + event.getClass().getTypeName());
        }
    ...
    private static void s_gameOver() {
        ...
        System.out.println("Made it to: Day %d of %d".formatted(environment.getGameTime(),
                environment.getGameLength()));
